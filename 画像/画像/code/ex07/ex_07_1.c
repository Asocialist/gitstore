...
int x,y,nx,ny;
   ...
   for (y=0;y<by;y++){
     for (x=bx;x<width;x++){
       ...
       for (iy=0;iy<ny;iy++){
  for (ix=0;ix<nx;ix++){
    sum+=imgin[0][x+ix][y+iy];
    ave=(int)((double)sum/(double)(nx*ny)+0.5);
  }
       }
       if (ave>255)
  ave=255;
  ...
  ave=0;
       for (iy=0;iy<nx;iy++){
  for (ix=0;ix<ny;ix++){
    imgout[0][x+ix][y+iy]=(unsigned char)ave;
  }
       }
       imgout[1][x][y]=128;
       imgout[2][x][y]=128;/*1,0*/
  }
   } 
    for (y=bx;y<height;y++){
     for (x=by;x<width;x++){
  ...
  for (ix=0;ix<nx;ix++){
    sum+=imgin[0][x+ix][y+iy];
    ave=(int)((double)sum/(double)(ny*nx)+0.5);
  }
       }
       if (ave>255)
  ave=255;
