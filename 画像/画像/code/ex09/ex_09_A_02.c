...
*/
  ...
  */
  ...
  //int  ave,sum;
  /* while(1){
      printf("ブロックサイズを入力してください(2,4,8,16)\n");
      scanf("%d",&block);
      double id , cal;
      id= log2(block);
      cal =(double) id -(int)id;
      if(cal ==0 && block<=16 && block>0){
	break;
      } else {
	printf("入力ミス、再入力してください\n");
      }
    } 
    for (y = 0; y < height; y += block) {
        for (x = 0; x < width; x += block) {
	      ...
	      ave = (int)((double)sum / (double)(block * block)+0.5);
            ...
            for (yi = 0; yi < block; yi++)
                for (xi = 0; xi < block; xi++)
		  imgout[0][x + xi][y + yi] = (unsigned char)ave;
...
for ( i = 0; i <=2; i++) {
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            if (i == 0) { imgout[i][x][y] = (imgin[i][x][y] / n) * n + n / 2; }
            else { imgout[i][x][y] = 128; }
        }
    }
    }*/
  int x,y,l,max=0;
  ...
  for(l=0;l<256;l++){
    ...
    printf("\n<最大度数>\n");
    printf("画素値 %3d:%5d",maxlevel,max);
    for(l=0;l<256;l++){
      fchange[l]=(int)((double)f[l]/(double)max*100.0+0.5);
    }
  printf("\n<度数表>\n");
  for(l=0;l<256;l++){
    printf("画素値%3d:  %5d-->%5d\n",l,f[l],fchange[l]);
