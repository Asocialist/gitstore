...
void processing(){
  ...
  */
  ...
  printf("<使用する４つの画素値>\n");
   ...
   x_temp[0]=dpoint[0]-(int)dpoint[0];
   y_temp[0]=dpoint[1]-(int)dpoint[1];
   x_temp[1]=1-x_temp[0];
   y_temp[1]=1-y_temp[0];

   value_temp[0]=x_temp[1]*(double)d[(int)dpoint[0]][(int)dpoint[1]]+x_temp[0]*(double)d[(int)dpoint[0]+1][(int)dpoint[1]];
   value_temp[1]=x_temp[1]*(double)d[(int)dpoint[0]][(int)dpoint[1]+1]+x_temp[0]*(double)d[(int)dpoint[0]+1][(int)dpoint[1]+1];
   value=(int)(y_temp[1]*value_temp[0]+y_temp[0]*value_temp[1]+0.5);
