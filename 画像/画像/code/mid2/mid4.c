...
//1 8*16
  bx=16;
       by=8;
   for (y=(height/4);y<(height/2);y+=by){
     for (x=0;x<width;x+=bx){
       sum=0;
       
       for (yi=0;yi<by;yi++){
  for (xi=0;xi<bx;xi++){
    sum+=imgin[0][x+xi][y+yi];
    ave=(int)((double)sum/(double)(bx*by)+0.5);
    if (ave>255)
  ave=255;
       else if (ave<0)
  ave=0;
  ...
  for (xi=0;xi<bx;xi++){
   ...
   //2 16*32
    bx=32;
       by=16;
    for (y=(2*b_height);y<(3*b_height);y+=by){
     for (x=0;x<width;x+=bx){
       sum=0;
  ...
  for (xi=0;xi<bx;xi++){
    sum+=imgin[0][x+xi][y+yi];
    ave=(int)((double)sum/(double)(by*bx)+0.5);
     if (ave>255)
  ave=255;
       else if (ave<0)
  ave=0;
  ...
  bx=64;
       by=32;
   for (y=3*b_height;y<height;y+=by){
     for (x=0;x<width;x+=bx){
       sum=0;
